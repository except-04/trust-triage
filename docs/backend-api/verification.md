...
## 2026-09-17 — 초기 모델·SHAP 프로세스 재사용

초기 분석마다 모델 파일을 다시 역직렬화하던 경로를 개선했다. 모델·SHAP 객체는 제한된 전용 프로세스에 보관하고, 파일별 Feature 추출은 별도 프로세스로 계속 격리한다. 모델 파일 변경·오류·시간 초과·재사용 한도 도달 시 전용 프로세스를 폐기하고 새로 준비한다. `BACKEND_REUSE_MODELS=false`로 이전 동작을 선택할 수 있다.

새 회귀 검증은 무해한 Feature·모델 대역으로 두 작업에서 모델·SHAP 준비가 각각 한 번만 일어나는지, 파일별 추출·모델 교체·실패 복구·종료 정리를 확인했다. 관련 통합 회귀 실행은 **811 통과, 92 건너뜀**이었다. 실제 팀 모델과 실제 PE를 로딩하거나 실행한 검증은 포함하지 않는다.
