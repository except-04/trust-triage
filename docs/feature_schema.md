...
- **해싱됨: 197개(39.4%)** → 추출기 로직상 "버킷 번호(Index) 숫자"는 재현 가능하나 원본 해싱 전 문자열(API명 등)로의 역추적은 **불가능**합니다. "이 카테고리가 중요하다"는 참고 힌트로만 사용하고, 설명 가능한 실제 API 그룹은 도메인 지식으로 백엔드에서 독립 설계합니다.

> **출력 이름 규칙 (Output Naming)**  
> EMBER 추출기는 사전이나 객체가 아닌 고정된 순서의 1차원 `float32` 배열을 반환합니다. 따라서 본 문서에 나열된 `timestamp`와 같은 사람이 읽기 쉬운 이름들은 실제 JSON/API 응답의 키로 출력되지 않으며, 모델 내/외부에서는 배열 인덱스 기반 위치 이름(`general[0]`, `header[2]` 등)을 사용합니다.
...
## 8. 파일명 기반 특징 (thrember에 없음, 자체 추가 제안 — 미구현)

> [!WARNING]
> 이 섹션의 특징은 최초 기획 시 멘토 피드백으로 제안되었으나, 현재 **구현되어 있지 않습니다.**
> 공식 모델(`baseline_model_lightgbm_tuned_500_4way.pkl`)은 오직 EMBER2024 원본 기반 500개 특징만 입력으로 받습니다.
...
- [ ] StringExtractor 76개 패턴 별도 이식 여부 (초기 기획이며, 현재는 공식 thrember 추출 및 Top-500 선택 정책을 따르므로 모델 입력 누락으로 해석하지 않음)
- [x] `pe_format_warnings`를 분석 난이도 신호로 정식 채택할지 (현재 `src/jrr/risk_signals.py`와 `backend_api/initial_analysis.py`에서 난이도 신호로 사용 중)
- [ ] `packing_suspected` 기준 확정 (초기 제안 항목으로 남아 있으며 별도 특징 채택 여부와 기준 결정 필요. 단, 현재 공식 모델의 필수 기능 누락으로 분류하지는 않음)
