...
- [x] 인식한 미지원 API 경고를 `UNSUPPORTED_API`로 구분 (`speakeasy_analyzer.py`, `tests/test_dynamic_analysis.py`). 모든 미지원 API 탐지를 보장하지는 않는다.
- [x] timeout·미지원 API 종료 전 확보한 부분 결과 보존
- [x] 실제 프로세스·네트워크 재현 한계 문서화 (2.1.3절)

아래는 현재 PE MVP 완료 조건과 구분하는 확장 후보다. 채택 시 구현·검증한다.

- [ ] Stub·Hook의 적용 여부와 버전이 기록됨
- [ ] 부분 결과에 실행 경로 Coverage 측정 추가
...
- [x] 실행 메타데이터 기록
- [x] timeout 종료 안전장치
- [x] 분석기의 선택적 로컬 raw report 저장 및 Worker 정규화 결과 보관 (운영 Worker는 `include_raw_report=False`; 전체 엔진 raw report 수집과 구분)
- [ ] 다른 팀 모듈이 사용할 출력 계약 리뷰 (코드/테스트 구현과 별개로 실제 팀 리뷰 완료 여부는 잔여 과제)
...
### 4단계 — 확장 채택 시 통합 평가와 운영 확인

- [ ] 동일 샘플의 정적·Speakeasy·CAPE·Qiling 결과 연결
- [ ] 도구별 비용과 실패율 기록
- [x] 확률 직접 판정·확률 구간 보류·JRR의 라우팅 비교 및 신호 제거 평가 ([평가 보고서](../../src/jrr/jrr_final_report.md))
- [ ] 심층분석 후 최종 오판 회수·비용 비교 (위 라우팅 평가에 포함되지 않음)
- [ ] 발표용 시나리오 작성 (데모 코드 존재와 별개로 실제 발표 시나리오/리뷰는 잔여 과제)
- [x] 현재 Speakeasy의 재현 한계와 미지원 범위 문서화 (2.1절 및 [사용 문서](speakeasy.md))
- [ ] CAPE/Qiling 도입 시 해당 도구의 실제 지원 범위·한계 문서화
